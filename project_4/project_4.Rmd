...
------------------------------------
 ...
 In an offline setting, recommending most popular items is the best strategy in terms of accuracy, while in a live environment this strategy is most likely the poorest. There are several researches that shows that offline and online evaluations often contradict each other.The obvious lack of predictive power of offline evaluation is the ignorance of human factor. These factors may strongly influence whether users are satisfied with recommendations, regardless of the recommendation’s relevance. 
 
 One of the widely used online recommender system evaluation method is A/B testing. Users are randomly split into two groups and they are offered slightly different experience. These way two different models can be tested and the best model can be selected using certain metrics (which depend on business objectives): click through rate, time spend watching videos, actual purchases etc.
